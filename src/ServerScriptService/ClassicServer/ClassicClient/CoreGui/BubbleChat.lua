...
local NEAR_BUBBLE_DISTANCE = 45	--previously 45
...
function this:CreateChatLineRender(instance, line, onlyCharacter, fifo, shouldAutoLocalize)
	...
	if billboardGui then
		...
		if UserFixBubbleChatText then
			...
			local height = numOflines * CHAT_BUBBLE_LINE_HEIGHT+CHAT_BUBBLE_PADDING
...
function this:BubbleChatEnabled()
	return game.ReplicatedStorage:WaitForChild("CoreGuiEnabled").bubblechat.Value
end
...
game:GetService("TextChatService").MessageReceived:Connect(function(message)
	...
	this:OnPlayerChatMessage(sender, message.Text, nil)
end)
