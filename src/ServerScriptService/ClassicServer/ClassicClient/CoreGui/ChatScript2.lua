...
local GuiRoot = script.Parent.Parent.symlink.Value
...
do
	...
	function Util.SetGUIInsetBounds(x, y)
		...
		script.Parent.Parent.symlink.Value.Size = UDim2.new(1,-x,1,-y)
	end
		...
		local container = Util.Create'Frame'
		{
			...
			elseif this.SendingPlayer and this.SendingPlayer ~= Player and this.PlayerChatType == Enum.PlayerChatType.Whisper then
				...
				local userNameDot = Util.Create'ImageLabel'
				{
					...
					BorderSizePixel = 0;
					Image = "rbxassetid://16080947161";--"rbxasset://textures/ui/chat_teamButton.png";
...
local function CreateChatBarWidget(settings)
	...
	local function HookUpEvents()
		...
		game:GetService("UserInputService").InputBegan:Connect(function(input,ignore)
			if not ignore and input.KeyCode == Enum.KeyCode.Slash then
				task.wait()
				this:FocusChatBar()
			end
		end)
				...
				if cText ~= "" then
					...
					--else
						...
						if currentMessageMode == 'Team' then
							if Player and Player.Neutral == true and not game.TextChatService.TextChannels:FindFirstChild("RBXTeam"..game.Players.LocalPlayer.TeamColor.Name) then
								this.ChatErrorEvent:fire("You are not in any team.")
							else
								--pcall(function() PlayersService:TeamChat(cText) end)
								--SayMessageRequest:FireServer(cText,"team")
								game.TextChatService.TextChannels:FindFirstChild("RBXTeam"..game.Players.LocalPlayer.TeamColor.Name):SendAsync(cText)
							end
						elseif currentMessageMode == 'Whisper' then
							if this.TargetWhisperPlayer then
								if this.TargetWhisperPlayer == Player then
									...
									--SayMessageRequest:FireServer(cText,this.TargetWhisperPlayer)
								end
							else
						...
						elseif currentMessageMode == 'All' then
							...
							--SayMessageRequest:FireServer(cText,"all")
							game.TextChatService.TextChannels:FindFirstChild("RBXGeneral"):SendAsync(cText)
						else
	...
	local function CreateChatWindow()
		local container = Util.Create'Frame'
		{
			...
			local scrollingFrame = Util.Create'ScrollingFrame'
			{
				...
				BackgroundColor3 = Color3.new(0, 0, 0);
				BackgroundTransparency = 1;
				BottomImage = "rbxassetid://16080947019";--"rbxasset://textures/ui/scroll-bottom.png";
				MidImage = "rbxassetid://16080946863";--"rbxasset://textures/ui/scroll-middle.png";
				TopImage = "rbxassetid://16080946662";--"rbxasset://textures/ui/scroll-top.png";
...
local function CreateChat()
	...
	game:GetService("TextChatService").MessageReceived:Connect(function(message)
		if not message.Status == Enum.TextChatMessageStatus.Success then
			return
		end
		if message.TextChannel.Name == "RBXGeneral" then
			this:OnPlayerChatted(Enum.PlayerChatType.All, game.Players:GetPlayerByUserId(message.TextSource.UserId), message.Text, "all")
		end
		if message.TextChannel.Name == "RBXTeam"..game.Players.LocalPlayer.TeamColor.Name then
			this:OnPlayerChatted(Enum.PlayerChatType.Team, game.Players:GetPlayerByUserId(message.TextSource.UserId), message.Text, "team")
		end
	end)
	...
	function this:CreateTouchDeviceChatButton()
		return Util.Create'ImageButton'
		{
			...
			Image = "rbxassetid://16080947285";--'rbxasset://textures/ui/Chat/Chat.png';
		};
	end
	...
	function this:Initialize()
		...
		this:CoreGuiChanged(ReplicatedStorage:WaitForChild("CoreGuiEnabled"):WaitForChild("chat").Value)
		...
		task.spawn(function()
			...
			while true do
				task.wait()
				local value = ReplicatedStorage:WaitForChild("CoreGuiEnabled"):WaitForChild("chat").Value
				if value ~= lastValue then
					this:CoreGuiChanged(Enum.CoreGuiType.Chat,value)
				end
				lastValue = value
			end
		end)
